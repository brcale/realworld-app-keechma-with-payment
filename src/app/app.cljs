(ns app.app
  (:require [keechma.next.controllers.router]
            ...
            [app.controllers.user.payment-form]
            [app.controllers.user.friends-list]
            ["react-dom" :as rdom]))
  ...
  {:keechma.subscriptions/batcher rdom/unstable_batchedUpdates
   ...
   {:user {:keechma.app/should-run? (role-eq? :user)
           ...
           {:settings-form #:keechma.controller {:type :user/settings-form
            ...
            :payment-form #:keechma.controller {:type :user/payment-form
                                                :params (page-eq? "payment")
                                                :deps [:router :jwt]}
            :editor-form #:keechma.controller {:type :user/editor-form
                                               :params (page-eq? "editor")
                                               :deps [:router :article :jwt]}
            
            :friends-list #:keechma.controller {:type :user/friends-list
                                                :params (page-eq? "payment")
